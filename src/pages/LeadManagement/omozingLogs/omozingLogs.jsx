...
import { ivrLogsColumn, leadsColumn, omozingLogsColumn, rmLogsColumn } from '../../../components/TableHeader';
import { useNavigate } from 'react-router-dom';
import { getIvrLogs, getLeads, getOmozingLogs, getRmLogs } from '../../../api-services/Modules/Leads';
...
const OMO_LOGS_STATUS_OPTIONS = [
    { value: "record added", label: "✅ Record added" },
    { value: "Rejected: take_home_salary is less than ₹40,000", label: "⚠️ Rejected" },
    { value: "Lead already exists in Zoho CRM", label: "⚠️ Lead already exists in Zoho CRM" },
];
...
const Leads = () => {
    ...
    const fetchLeads = useCallback(async () => {
        setLoading(true);
        try {
            const res = await getOmozingLogs(
                query.filter_date,
                query.startDate,
                query.endDate
            );
    ...
    useEffect(() => {
        ...
        const calculateCount = (list, messagePart) => {
            return list.filter(lead => {
                const msg = lead?.lender_response?.omozing?.message || '';
                return msg.trim().includes(messagePart);
            }).length;
        };
        setSummaryMetrics({
            totalLeads: _list.length,
            successCount: calculateCount(_list, 'record added'),
            rejectCount: calculateCount(_list, 'Rejected: take_home_salary is less than ₹40,000'),
            invalidDataCount: calculateCount(_list, 'Lead already exists in Zoho CRM')
        });
    ...
    return (
        <>
            ...
            />
            <DataTable
                columns={omozingLogsColumn({ handleEdit })}
                data={tableData}
                ...
                statusFilterOptions={OMO_LOGS_STATUS_OPTIONS} // Pass the dynamic options array
